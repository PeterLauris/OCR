	...
	l_int32  which)  /* 1, 2, 3, 4 */
{
	...
	if (which == 1) ht_flag = 1;
	else if (which == 2) ws_flag = 1;
	else if (which == 3) text_flag = 1;
	else if (which == 4) block_flag = 1;
	...
	pixDisplayWriteFormat(pixt1, L_MAX(ws_flag, L_MAX(ht_flag, block_flag)), IFF_PNG);
	...
	pixZero(pixht, &zero);
	if (zero) fprintf(stderr, "No halftone parts found\n");
...
int main_segmentation()
{
	PIX* pixs_paper = pixRead("..\\..\\..\\images\\test-set\\source.jpg");
	if (!pixs_paper) {
		printf("ERROR: File not found");
		return 1;
	}

	PIX *pix_paper_grayscale = pixScaleRGBToGrayFast(pixs_paper, 1, COLOR_GREEN);
	printf("Create color grayscale image\n");
	pixWrite("tmp/color.grayscale.tif", pix_paper_grayscale, IFF_TIFF_G4);
	PIX *pix_paper_1bpp = pixThreshold8(pix_paper_grayscale, 1, 0, 0);
	printf("Create color 1bpp image\n");
	pixWrite("tmp/color.1bpp.tif", pix_paper_1bpp, IFF_TIFF_G4);

	//DoPageSegmentation(pix_paper_1bpp, 1);

	//for (int i = 1; i <= 4; i++)
		DoPageSegmentation(pix_paper_1bpp, 1); //bildes j?padod 1bpp

	pixDestroy(&pix_paper_1bpp);
	pixDestroy(&pix_paper_grayscale);
