...
{
	PIX* pixs_paper = pixRead("test1.png");
	if (!pixs_paper) {
	...
	pixWrite("color.grayscale.tif", pix_paper_grayscale, IFF_TIFF_G4);
	PIX *pix_paper_1bpp = pixThreshold8(pix_paper_grayscale, 1, 0, 0);
	printf("Create color 1bpp image\n");
	pixWrite("color.1bpp.tif", pix_paper_1bpp, IFF_TIFF_G4);

	//DoPageSegmentation(pix_paper_1bpp, 1);

	//for (int i = 1; i <= 4; i++)
		DoPageSegmentation(pixs_paper, 1); //bildes j?padod 1bpp
